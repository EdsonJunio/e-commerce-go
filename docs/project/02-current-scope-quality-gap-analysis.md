...
| ECOM-003.1 | done | `fix/ecom-003-1-sku-schema-mapping` | Independent review approved all acceptance criteria and follow-up review confirmed the documentation correction. `ProductSKU` maps the migrated table without an inventory column and decodes JSONB attributes; focused red/green, PostgreSQL, full, race, vet, build, formatting, coverage, and migration-up gates passed. Lint and OpenAPI remain blocked by documented pre-existing limitations; security scanners are unavailable. | 2026-09-26 |
| ECOM-003.2 | ready | `fix/ecom-003-2-sku-list-filters` | ECOM-003.1 is complete; SKU listing still uses raw map filters, mismatched handler/repository keys, permissive parsing, and an invalid `sku_id` column. | 2026-09-26 |
